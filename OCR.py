    ...
    def ocr(self):
        ...
        while not self.stopped:
            if self.exchange is not None:  # Defends against an undefined VideoStream reference
                ...
                print (pytesseract.image_to_string(frame, lang=self.language)) 
                engine.say(pytesseract.image_to_string(frame, lang=self.language))
                engine.runAndWait()
...
def ocr_stream(crop: int, source: int = 0, view_mode: int = 1, language=None):
    """
